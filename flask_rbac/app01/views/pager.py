...
class Pagination(object):
    ...
    def __init__(self, current_page, total_count, base_url, params, per_page_count=10, max_pager_count=11):
        ...
        if current_page <= 0:
            current_page = 1
        self.current_page = current_page  # 当前页面
        self.total_count = total_count  # 数据总条数
        self.per_page_count = per_page_count  # 每页显示10条数据
        self.max_page_num = math.ceil(total_count / per_page_count)  # 页面上应该显示的最大页码
        self.max_pager_count = max_pager_count  # 页面上默认显示11个页面（当前页在中间）
    ...
    def page_html(self):
        ...
        if self.current_page != 1:
            ...
            page_html_list.insert(1,
                                  '<li><a href="%s?%s" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>' % (
                                      self.base_url, urlencode(self.params)))

        if self.current_page != self.max_page_num:
            self.params['page'] = self.current_page + 1
            page_html_list.append(
                '<li><a href="%s?%s" aria-label="Next"><span aria-hidden="true">&raquo;</span></a></li>' % (
                    self.base_url, urlencode(self.params)))
